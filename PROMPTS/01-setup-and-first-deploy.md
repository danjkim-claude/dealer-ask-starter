...
5. Write data/PROFILE.md for the store_day table from real queries through a small tsx script in scripts/ that starts with import "./env"; (like every other script in that folder, so it reads .env.local and talks to the database you just loaded) and uses lib/db/client.ts readOnly(): row count, every column's type and null rate, min and max of report_date, rows per store with each store's last report date, distinct values of any low-cardinality column, and the unique key with proof (count vs count distinct). Do not read docs/06-gotchas-and-refusals.md; I want the profile to find things on its own. End the profile with two one-line answers in your own words: "What is one row in this table?" and "How fresh is this data?"
