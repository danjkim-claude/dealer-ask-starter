...
5. Run npm run verify. Do not run any git command: this laptop may not have git, and publishing is done by hand in GitHub Desktop. Instead, finish by telling me in one line to open GitHub Desktop, type the commit message, click Commit to main, then click Push origin. The message depends on what happened: if you applied colours from the site use "Brand: colours from our website"; if you kept the defaults because the site could not be read use "Brand: kept defaults, site could not be read". Say that the live site rebuilds about a minute after that push.
...
- If the site could not be fetched, DESIGN.md's Brand source line says so and the header stays teal. That is a pass, not a failure.
